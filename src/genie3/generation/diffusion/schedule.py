...
Provides variance schedule functions for diffusion processes (cosine, linear, etc.).
"""
...
def get_betas(n_timestep: int, schedule: str) -> torch.Tensor:
    """
...
def cosine_beta_schedule(n_timestep: int) -> torch.Tensor:
    """
    ...
    return torch.concat([torch.zeros((1,)), torch.clip(betas, 0, 0.999)])
