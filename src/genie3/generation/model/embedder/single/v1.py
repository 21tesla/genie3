"""
V1 single feature embedder.

Generates single (per-residue) representations for the V1 model architecture.
"""
...
class V1SingleFeatureNet(nn.Module):
    ...
    def __init__(
        ...
        """
        Initialize V1 single feature embedder.
    ...
    def forward(self, batch: Dict[str, torch.Tensor], t: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            batch: Batch data with token, chain, sequence, and atomization information
        ...
        residue_index_emb = sinusoidal_encoding(
            batch["residue_index"], self.max_n_res, self.c_pos_emb
        )
        ...
        restype = batch["gt_restype"] * batch["cond_seq_mask"][..., None]
        ...
        return self.linear(
            torch.cat(
                [
                    ...
                    batch["atom_type"],
                    batch["atom_symbol"],
                    batch["is_atomized"].unsqueeze(-1),
                    batch["token_mask"].unsqueeze(-1),
                    batch["token_struct_mask"].unsqueeze(-1),
                    batch["token_struct_frame_mask"].unsqueeze(-1),
                    cond_group_mask.unsqueeze(-1),
                    batch["cond_seq_mask"].unsqueeze(-1),
                    batch["cond_struct_mask"].unsqueeze(-1),
                    batch["cond_struct_frame_mask"].unsqueeze(-1),
                    batch["cond_interface_mask"].unsqueeze(-1),
                ],
                dim=-1,
            )
        ) * batch["token_mask"].unsqueeze(-1)
